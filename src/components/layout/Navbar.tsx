...
const PRODUCT_LINKS = [
  { label: "Tile", href: "/products#tile" },
  { label: "AC", href: "/products#ac" },
];
                ...
                link.label === "Our Products" ? (
                  <div key={link.label} className="relative group">
                    <Link
                      ...
                      href={link.href}
                      className="relative inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold text-white/80 hover:text-white transition-colors duration-200 cursor-pointer will-change-transform"
                    >
                      {link.label}
                      <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={2} className="w-3 h-3 group-hover:rotate-180 transition-transform duration-200">
                    ...
                    <div className="absolute left-1/2 -translate-x-1/2 top-full pt-3 opacity-0 invisible translate-y-1 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-200">
                      <div
                        className="min-w-[170px] rounded-xl bg-[#0E1A2B]/95 border border-white/10 shadow-[0_16px_40px_-12px_rgba(0,0,0,0.6)] p-2"
                        style={{ backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)" }}
                      >
                        {PRODUCT_LINKS.map((product) => (
                          <Link
                            key={product.label}
                            href={product.href}
                            className="block px-4 py-2.5 rounded-lg text-sm font-semibold text-white/80 hover:text-white hover:bg-white/8 transition-colors duration-200 cursor-pointer"
                          >
                            {product.label}
                          </Link>
                        ))}
      ...
      >
        ...
        >
          <div>
            ...
            <p className="text-[#7A8FA6] text-xs font-mono">076 014 2500</p>
          </div>
        </div>
      </div>
    </>
